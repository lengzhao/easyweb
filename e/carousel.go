...
func (b *CarouselElement) String() string {
	...
	node.AddAttribute("id", b.sid)

	btns := NewNode("div").AddAttribute("class", "carousel-indicators")
	for i := range b.slides {
		btn := NewNode("button").AddAttribute("type", "button").AddAttribute("data-bs-target", "#"+b.sid).AddAttribute("data-bs-slide-to", fmt.Sprint(i))
		if i == 0 {
	...
	node.AddChild(imgs)
	btn1 := NewNode("button").AddAttribute("class", "carousel-control-prev").AddAttribute("type", "button").AddAttribute("data-bs-target", "#"+b.sid).AddAttribute("data-bs-slide", "prev")
	...
	node.AddChild(btn1)
	btn2 := NewNode("button").AddAttribute("class", "carousel-control-next").AddAttribute("type", "button").AddAttribute("data-bs-target", "#"+b.sid).AddAttribute("data-bs-slide", "next")
